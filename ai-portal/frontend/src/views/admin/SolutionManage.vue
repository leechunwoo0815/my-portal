<template>
  <div class="manage-view">
    ...
    <el-table :data="list" :row-key="(row: any) => row.id" v-loading="loading" style="width:100%">
      ...
      <el-table-column label="操作" width="180" fixed="right">
        <template #default="s">
          ...
          <el-popconfirm title="确定删除这个解决方案？" confirm-button-text="确定" cancel-button-text="取消" @confirm="handleDelete(s.row.id)">
            <template #reference>
    ...
    <el-dialog :title="isEdit?'编辑解决方案':'新增解决方案'" v-model="dialogVisible" width="90vw" top="5vh" :close-on-click-modal="false">
      <el-form :model="form" label-width="80px">
        ...
        <el-form-item label="内容">
          <MilkdownEditor v-model="form.content" module="solutions" />
        </el-form-item>
        <el-form-item label="封面图"><CoverUpload v-model="form.cover_image" module="solutions" /></el-form-item>
...
import { adminListSolutions, createSolution, updateSolution, deleteSolution } from '@/api/solutions'
...
const {
  ...
  fetchList: (params) => adminListSolutions(params),
  createItem: (data) => createSolution(data),
  updateItem: (id, data) => updateSolution(id, data),
  deleteItem: (id) => deleteSolution(id),
  ...
  deleteConfirmMessage: '确定删除这个解决方案？',
  entityName: '解决方案',
})
