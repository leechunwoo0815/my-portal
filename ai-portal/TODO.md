...
| # | 问题 | 说明 | 状态 |
|---|------|------|------|
| 1 | 通知端点路径一致性 | 前端 `notification.ts` 请求路径与后端路由需确认完全匹配 | ⬜ |

### P1 — 体验问题（全部已完成）

| # | 问题 | 说明 | 状态 |
|---|------|------|------|
| 1 | Token 自动刷新 | access_token (30min) + refresh_token (30d) + 前端自动续期 | ✅ |
| 2 | 仪表盘图表数据 | 4 个 ECharts 图表从后端 /stats/charts 获取真实数据 | ✅ |
| 3 | 后端全文搜索 | SQL 级分页 + title/summary/tags 搜索 + /suggest 端点 | ✅ |
| 4 | 私信支持图片 | message_type + image_url + 前端图片上传/预览 | ✅ |
| 5 | WebSocket 通知 | 实时推送 + 心跳保活 + 自动重连 + 桌面弹窗 | ✅ |
